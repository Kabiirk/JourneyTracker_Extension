...
describe('JourneyList component', () => {
  ...
  test('Record text functionality', async () => {
    const page = await browser.newPage();
    await page.goto(`chrome-extension://${EXTENSION_ID}/popup.html`);
    const client = await page.target().createCDPSession();
    await client.send('Network.clearBrowserCookies');
    ...
    await page2.goto('https://stackoverflow.com/questions/71942822/materialui-v5-how-to-style-autocomplete-options');
    await setTimeout(3000);

    await page2.waitForSelector('p');

    await page2.evaluate(() => {
      const paragraph = document.querySelector('p'); // Assuming you want to select the first paragraph
    ...
    await page.mouse.click(10, 50, { button: 'right' }); // Adjust the coordinates as needed
    await page.waitForSelector('.context-menu');

    await setTimeout(6000);

    // Scroll into the highlighted text
    // await page.evaluate(() => {
    //   const paragraph = document.querySelector('p'); // Assuming you want to select the first paragraph
    //   paragraph.scrollIntoView({ behavior: 'smooth', block: 'center' });
    // });
  ...
  test('Create a new journey', async () => {
    ...
    const inputSelector = '.MuiInputBase-root'; // Replace with the actual class name
    await page.waitForSelector('.MuiInputBase-root', { timeout: 5000 }); // Wait for the input to appear
  ...
  // Find all buttons with the class 'MuiButtonBase-root'
    ...
    // const buttonSelector = '[data-testid="your-button-testid"]'; // Replace with the actual data-testid value
    // await page.waitForSelector(buttonSelector);
    // await page.click(buttonSelector);
